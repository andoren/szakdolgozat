...
CREATE UNIQUE INDEX `FK_partner_uniqueBytelephely` ON `iktato`.`partner` (`name` ASC, `telephely` ASC, `deleted` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- Table `iktato`.`ikonyvpartnerei`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `iktato`.`ikonyvpartnerei` ;

CREATE TABLE IF NOT EXISTS `iktato`.`ikonyvpartnerei` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `partner_id` INT NOT NULL,
  `partnerugyintezo_id` INT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_ikonyvpartnerei_partner`
    FOREIGN KEY (`partner_id`)
    REFERENCES `iktato`.`partner` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ikonyvpartnerei_ugyintezo`
    FOREIGN KEY (`partnerugyintezo_id`)
    REFERENCES `iktato`.`partnerugyintezo` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_ikonyvpartnerei_partner_idx` ON `iktato`.`ikonyvpartnerei` (`partner_id` ASC) VISIBLE;

CREATE INDEX `fk_ikonyvpartnerei_ugyintezo_idx` ON `iktato`.`ikonyvpartnerei` (`partnerugyintezo_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `iktato`.`ikonyv` (
  ...
  CONSTRAINT `fk_ikonyv_partner`
    FOREIGN KEY (`partner`)
    REFERENCES `iktato`.`ikonyvpartnerei` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `iktato`.`prevYearIkonyv` (`ikonyvid` INT, `iktatoszam` INT, `irany` INT, `targy` INT, `hivszam` INT, `year` INT, `erkezett` INT, `hatido` INT, `telephely` INT, `deleted` INT, `szoveg` INT, `partnerid` INT, `partnername` INT, `partnerugyintezoid` INT, `partnerugyintezoname` INT, `ugyintezoid` INT, `ugyintezoname` INT, `csoportid` INT, `csoportname` INT, `csoportshortname` INT, `jellegid` INT, `jellegname` INT, `doccount` INT);
...
USE `iktato`;
DROP procedure IF EXISTS `iktato`.`modifyuser`;
...
-- -----------------------------------------------------
-- procedure AddYearAndActivate
-- -----------------------------------------------------

USE `iktato`;
DROP procedure IF EXISTS `iktato`.`AddYearAndActivate`;
...
-- -----------------------------------------------------
-- procedure getdocsbyikonyvid
-- -----------------------------------------------------

USE `iktato`;
DROP procedure IF EXISTS `iktato`.`getdocsbyikonyvid`;
...
-- -----------------------------------------------------
-- procedure getalltelephely
-- -----------------------------------------------------

USE `iktato`;
DROP procedure IF EXISTS `iktato`.`getalltelephely`;
...
,i.szoveg as szoveg, p.id as partnerid, p.name as partnername, pu.id as partnerugyintezoid, pu.name as partnerugyintezoname,
 u.id as ugyintezoid, u.name as ugyintezoname,cs.id as csoportid, cs.name as csoportname, cs.shortname as csoportshortname,j.id as jellegid, j.name as jellegname, IFNULL(COUNT(docs.ikonyv_id),0) as doccount
 from ikonyv as i 
 join ikonyvpartnerei ip on (i.partner = ip.id) 
 join partner p on(ip.partner_id= p.id)
 left join partnerugyintezo pu on (ip.partnerugyintezo_id = pu.id)
 join csoport cs on (i.csoport = cs.id)
 join jelleg j on (i.jelleg = j.id) join ugyintezo u on(i.ugyintezo = u.id)
 Left join (select * from ikonyvdocs where deleted = 0) docs on (i.id = docs.ikonyv_id) 
  where  yearid = (select id from evek where active = 1) group by i.id order by i.iktatottid, i.valaszid;
...
USE `iktato`;
CREATE  OR REPLACE VIEW `prevYearIkonyv` AS select i.id as ikonyvid,i.iktatoszam as iktatoszam,i.irany as irany, i.targy as targy,i.hivszam as hivszam,i.yearid as year, i.erkezett as erkezett, i.hatido as hatido, i.telephely as telephely, i.deleted as deleted
,i.szoveg as szoveg, p.id as partnerid, p.name as partnername, pu.id as partnerugyintezoid, pu.name as partnerugyintezoname,
 u.id as ugyintezoid, u.name as ugyintezoname,cs.id as csoportid, cs.name as csoportname, cs.shortname as csoportshortname,j.id as jellegid, j.name as jellegname, IFNULL(COUNT(docs.ikonyv_id),0) as doccount
 from ikonyv as i 
 join ikonyvpartnerei ip on (i.partner = ip.id) 
 ...
 join jelleg j on (i.jelleg = j.id) join ugyintezo u on(i.ugyintezo = u.id)
 Left join (select * from ikonyvdocs where deleted = 0) docs on (i.id = docs.ikonyv_id) 
 group by i.id order by i.iktatottid, i.valaszid;
USE `iktato`;
insert into privilege(privilege)value("admin");
insert into privilege(privilege)value("user");
call addUser('admin','kiscicakiscica','Administrator',1,@v);
call addyear(YEAR(CURDATE()),1,@v);
