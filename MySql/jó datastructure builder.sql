...
select generatedIktSzam into iktszam from createIktSzam where id = parentid_b;
...
DELIMITER $$
USE `iktato`$$
CREATE PROCEDURE `AddDoc` (in name_b varchar(250),in ext_b varchar(10),in path_b text,in ikt_id_b int ,in created_by_id int,size_b double ,out newid_b int)
BEGIN
	Declare newDocId int;
	insert into doc(name,ext,path)value(name_b,ext_b,path_b);
    select MAX(id) into newDocId from doc;
    insert into ikonyvdocs(ikonyv_id,doc_id,created_by,size)value(ikt_id_b,newDocId,created_by_id,size_b);
END$$
...
BEGIN
		select Distinct(u.id) as id,u.fullname as fullname, u.username as username from user u   where deleted = 0;
END$$
...
BEGIN
 select d.id, d.name, d.ext, d.path from ikonyvdocs inner join doc d on (ikonyvdocs.doc_id = d.id) where ikonyvdocs.ikonyv_id = ikonyvid_b;
END$$
