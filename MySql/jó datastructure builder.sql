...
CREATE UNIQUE INDEX `name_UNIQUE` ON `iktato`.`ugyintezo` (`name` ASC) VISIBLE;
...
DELIMITER $$
USE `iktato`$$
CREATE PROCEDURE `AddTelephely` (in name_b varchar(100))
BEGIN
	insert into telephely(name)value(name_b);
END$$
...
DELIMITER $$
USE `iktato`$$
CREATE PROCEDURE `AddUgyintezo` (in name_b varchar(100), in telephely_b int,in created_by_b int )
BEGIN
...
DELIMITER $$
USE `iktato`$$
CREATE PROCEDURE `AddUser` (in username_b varchar(45),in password_b varchar(250), in fullname_b varchar(120), in privilege_b tinyint)
BEGIN
...
BEGIN
	declare hasRow int;
    select count(csoport) into hasRow from ikonyv where csoport = id_b and deleted = 0;
    if(hasRow != 0) then update csoport set deleted = 1 and deleted_by = deleter_b and deleted_at = current_timestamp() where id = id_b;
    /*
    else
    
	  SIGNAL SQLSTATE  '02000';
      SET MESSAGE_TEXT := 'Nem lehet törölni csoportot még van a nevén iktatás!';
    */
    end if;
END$$
...
BEGIN
	declare hasRow int;
    select count(jelleg) into hasRow from ikonyv where jelleg = id_b and deleted = 0;
    if(hasRow = 0) then update jelleg set deleted = 1 , deleted_by = deleter_b , deleted_at = current_timestamp() where id = id_b;
    /*
    else 
	   SIGNAL SQLSTATE  '02000';
      SET MESSAGE_TEXT = 'Nem lehet törölni jelleget még van a nevén iktatás!';
    */
    end if;
END$$
...
DELIMITER $$
USE `iktato`$$
CREATE PROCEDURE `DelTelephely` (in id_b int)
BEGIN
	declare hasRow int;
    select count(*) into hasRow from ikonyv where telephely = id_b and deleted = 0;
    if(hasRow = 0) then delete from telephely where id = id_b;
   /*
   else
      SIGNAL SQLSTATE  '02000';
      SET MESSAGE_TEXT = 'Nem lehet törölni telephelyet még van a nevén iktatás!';
       */
    end if;
...
BEGIN
	update ugyintezo set deleted = 1 and deleted_by = deleter_b and deleted_at = current_timestamp() where id = id_b;
END$$
...
BEGIN
 update user set deleted = 1 and deleted_by = deleter_b and deleted_at = current_timestamp() where id = user_id_b;
END$$
...
BEGIN
	select * from currentYearIkonyv where id = id_b;
END$$
...
BEGIN
	select t.id as id, t.name as name from felh_telephely f inner join telephely t on (f.telephely_id = t.id)where f.user_id = user_b ;
END$$
...
BEGIN
	select * from ugyintezo where telephely = telephely_b;
END$$
...
BEGIN
	select u.username,u.fullname,p.privilege from user u inner join privilege p on (privilege = privilege.id) where u.deleted = 0;
END$$
...
BEGIN
 select ikonyvid, iktatoszam from currentYearIkonyv where telephely = telephely_b;
END$$
...
BEGIN
	update jelleg set name = name_b and shortname = shortname_b where id = id_b;
END$$
...
BEGIN
	select * from partner where telephely = telephely_b;
END$$
...
BEGIN
 select u.id, u.name from partnerugyintezokapcsolo p inner join partnerugyintezo u on(p.partnerugyintezoid = u.id) where p.partnerid = partner_b;
END$$
...
DELIMITER $$
USE `iktato`$$
CREATE PROCEDURE `getusers`(in user_b int)
BEGIN
	select u.id as id,u.fullname as fullname, u.username as username from felh_telephely f inner join user u on 
    (f.user_id = u.id) where f.telephely_id in ((select distinct id from felh_telephely where user_id = user_B));
END$$
